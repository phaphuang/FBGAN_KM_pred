...
class WGAN_LangGP():
    def __init__(self, batch_size=16, lr=0.0001, num_epochs=1000, seq_len=512, data_dir="./data/new_brenda_df_data.csv",
    ...
    def load_data(self, datadir):
        max_examples = 1e6
        self.data, self.charmap, self.inv_charmap, self.ecfp_lines, self.logkm_list = utils.language_helpers.load_dataset_ecfp(
    ...
    def disc_train_iteration(self, real_data, ecfp_data, logkm_data):
        self.D_optimizer.zero_grad()

        fake_data = self.sample_generator(self.batch_size)
        d_fake_pred, _ = self.D(fake_data)
        d_fake_err = d_fake_pred.mean()
        d_real_pred, logkm_pred = self.D(real_data)
        ...
        d_err = d_fake_err - d_real_err + gradient_penalty + 10 * logkm_err
        d_err.backward()
        self.D_optimizer.step()

        #### Calculate Fingerprint Similarity ####
        try:
            seqs = (fake_data.data).cpu().numpy()
            decoded_seqs = [AllChem.GetMorganFingerprintAsBitVect(
                Chem.rdmolfiles.MolFromSequence(decode_one_seq(seq, self.inv_charmap)), 3, nBits=1024).ToBitString() \
                for seq in seqs
            ]

            sim_lists = [DataStructs.FingerprintsSimilarity(x,y) for x,y in zip(ecfp_data, decoded_seqs)]
            #print("Similarity List: ", sim_lists)
            sim_err = np.mean(np.array(sim_lists))
        except Exception as e:
            sim_err = 0

        return d_fake_err.data, d_real_err.data, gradient_penalty.data, d_err, logkm_err.data, sim_err
    ...
    def gen_train_iteration(self):
        ...
        dg_fake_pred, _ = self.D(g_fake_data)
    ...
    def train_model(self, load_dir):
        ...
        for epoch in range(self.n_epochs):
            for idx in range(n_batches):
                ...
                ecfp_data = np.array(self.ecfp_lines[idx*self.batch_size:(idx+1)*self.batch_size])

                logkm_data = torch.Tensor(np.array(self.logkm_list[idx*self.batch_size:(idx+1)*self.batch_size]))
                logkm_data = to_var(logkm_data)

                d_fake_err, d_real_err, gradient_penalty, d_err, logkm_err, sim_err = self.disc_train_iteration(real_data, ecfp_data, logkm_data)
                ...
                logkm_losses.append(logkm_err)
                sim_losses.append(sim_err)
                ...
                if counter % 10 == 9:
                    summary_str = 'Iteration [{}/{}] - loss_d: {}, loss_g: {}, w_dist: {}, grad_penalty: {}, loss_km: {}, similarity: {}'\
                        .format(counter, total_iterations, (d_err.data).cpu().numpy(),
                        (g_err.data).cpu().numpy(), ((d_real_err - d_fake_err).data).cpu().numpy(), gp_np, logkm_err, sim_err)
                    print(summary_str)
                    ...
                    plot_losses([logkm_losses], ["logkm"], self.sample_dir + "logkm.png")

                    plot_losses([sim_losses], ["similarity"], self.sample_dir + "similarity.png")
                counter += 1
